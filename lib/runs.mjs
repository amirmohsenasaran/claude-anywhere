...
export class Run {
  ...
  enqueue(text) {
    ...
    this.push({ t: 'queued', id: item.id, text });
  ...
  async setControls({ permissionMode, model, effort }) {
    ...
    if (permissionMode && PERMISSION_MODES.includes(permissionMode)) { await this.query.setPermissionMode(permissionMode); applied.permissionMode = permissionMode; }
...
const userMessage = (text) => ({ type: 'user', message: { role: 'user', content: text }, parent_tool_use_id: null, session_id: '' });
...
export function startRun({ sessionId, cwd, prompt, model, permissionMode, effort }) {
  ...
  run.push({ t: 'prompt', id: firstId, text: prompt, at: run.startedAt }); // so a client that reloads mid-turn sees the question too
  ...
  async function* inputStream() {
    ...
    while (true) {
      if (run.queue.length) {
        ...
        run.inTurn = true;
        run.push({ t: 'prompt', id: item.id, text: item.text, at: Date.now() });
        yield userMessage(item.text);
        continue;
      }
      ...
      for await (const msg of q) {
        switch (msg.type) {
          ...
          case 'result':
            ...
            if (!run.queue.length) run.closing = true;
            run.wake?.();

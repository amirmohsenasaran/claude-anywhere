...
const mcpOf = async (q, name = MCP_NAME) => (await q.mcpServerStatus()).find((s) => s.name === name);

// Any http MCP server Claude Code knows, not only Houshyar24's: the Connectors list
// offers it to each. `again` forgets the token first - a sign-in that went stale, or
// one made as the wrong person, is replaced rather than reported as connected.
export async function startMcpSignIn(redirectUri, { name = MCP_NAME, again = false } = {}) {
  if (signIn?.state === 'waiting') await close(signIn.q);
  const q = openQuery();
  try {
    const now = await mcpOf(q, name);
    if (!now) throw new Error(name + ' is not an MCP server Claude Code knows here.');
    if (again) await q.mcpClearAuth(name);
    else if (now.status === 'connected') { await close(q); signIn = { name, state: 'connected', at: Date.now() }; return { state: 'connected' }; }
    const r = await q.mcpAuthenticate(name, redirectUri || undefined);
    if (!r?.authUrl) { await close(q); if ((await mcpConnection(name, true)) === 'connected') return { state: 'connected' }; throw new Error('Claude Code did not start a sign-in for ' + name + '.'); }
    // Houshyar24 may refuse our address; Claude Code then falls back to its own listener.
    const mine = signIn = { q, name, authUrl: r.authUrl, state: 'waiting', at: Date.now(), redirect: r.redirectScheme === 'custom' ? redirectUri : '' };
    (async () => {
      while (mine === signIn && mine.state === 'waiting') {
        await new Promise((ok) => setTimeout(ok, 1500));
        if (Date.now() - mine.at > SIGN_IN_FOR) { mine.state = 'expired'; break; }
        try { const s = await mcpOf(q, name); if (s?.status === 'connected') mine.state = 'connected'; else if (s?.status === 'failed') { mine.state = 'failed'; mine.error = s.error || 'failed'; } } catch (e) { mine.state = 'failed'; mine.error = e.message; }
      }
      await close(q);
    })();
...
export const mcpSignInStatus = () => (signIn ? { name: signIn.name, state: signIn.state, error: signIn.error || '' } : { state: 'none' });
...
export async function submitMcpCallback(url) {
  ...
  return signIn.q.mcpSubmitOAuthCallbackUrl(signIn.name, url);
}
...
export async function finishMcpSignIn(query) {
  ...
  try { await mine.q.mcpSubmitOAuthCallbackUrl(mine.name, mine.redirect + query); } catch (e) { if (/still waiting/i.test(e.message)) return 'rejected'; mine.state = 'failed'; mine.error = e.message; return 'failed'; }
...
export async function mcpConnection(name = MCP_NAME, fresh = false) {
  if (name !== MCP_NAME) { const q = openQuery(); try { return (await mcpOf(q, name))?.status || 'none'; } catch { return 'unknown'; } finally { await close(q); } }
  if (signIn?.name === MCP_NAME && signIn.state === 'connected') return 'connected';
  if (!fresh && Date.now() - connCache.at < 30000) return connCache.status;

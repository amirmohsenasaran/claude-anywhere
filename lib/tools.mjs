...
// listens on a localhost port for the redirect, then keeps the token where it keeps its
// own. The app opens that URL and waits for the server to come up connected. The query
// has to stay open meanwhile - closing it closes the listener - so it is held here for
// as long as the sign-in may take.
...
export async function startMcpSignIn() {
  ...
  try {
    ...
    const r = await q.mcpAuthenticate(MCP_NAME);
    if (!r?.authUrl) throw new Error('Claude Code did not start a sign-in for ' + MCP_NAME + '.');
    const mine = signIn = { q, authUrl: r.authUrl, state: 'waiting', at: Date.now() };
    (async () => {

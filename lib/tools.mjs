...
const mcpEntry = () => { try { return JSON.parse(read(path.join(home, '.claude.json')) || '{}').mcpServers?.[MCP_NAME] || null; } catch { return null; } };
export async function mcpStatus() {
  ...
  return { name: MCP_NAME, url, added: !!entry && entry.url === url, stale: !!entry && entry.url !== url, cli: !!which('claude') };
}

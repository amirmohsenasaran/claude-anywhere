...
let cache = { at: 0, models: null, error: '', from: '' };
let inFlight = null;
let cachePath = '';
...
export function useCache(dir) {
  ...
  try {
    ...
    if (Array.isArray(disk.models) && disk.models.length) cache = { at: disk.at || 0, models: disk.models, error: '', from: 'disk' };
  } catch {}
}
...
function refresh(env) {
  ...
  inFlight = ask(env)
    .then((models) => {
      if (!models) return;
      cache = { at: Date.now(), models, error: '', from: 'cli' };
      if (cachePath) { try { fs.mkdirSync(path.dirname(cachePath), { recursive: true }); fs.writeFileSync(cachePath, JSON.stringify({ at: cache.at, models }, null, 2)); } catch {} }
    })
...
/** The cached list, with a refresh started in the background when it is a day old. */
export function list({ env, force = false } = {}) {
  if (force || !cache.models || Date.now() - cache.at > DAY) refresh(env);
  return { models: cache.models || FALLBACK, at: cache.at, from: cache.models ? cache.from : 'fallback', error: cache.error, defaultEffort: DEFAULT_EFFORT };
}
